...
public class AmogusFeature extends Feature<NoneFeatureConfiguration> {
	public static final AmogusFeature FEATURE = (AmogusFeature) new AmogusFeature().setRegistryName("bonk:amogus");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public AmogusFeature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("bonk", "amogus"));
		if (template == null)
			return false;
